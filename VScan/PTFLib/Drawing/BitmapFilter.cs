...
namespace PremierTaxFree.PTFLib.Drawing
{
    ...
    public class BitmapFilter
    {
        ...
        public static bool Colore(Bitmap b, int red, int green, int blue)
        {
        ...
        public static bool EdgeDetectDifference(Bitmap b, byte nThreshold)
        {
            ...
            unsafe
            {
                ...
                for (int y = 1; y < b.Height - 1; ++y)
                {
                    ...
                    for (int x = 3; x < nWidth - 3; ++x)
                    {
                        int nPixelMax = Math.Abs((p2 - stride + 3)[0] - (p2 + stride - 3)[0]);
                        int nPixel = Math.Abs((p2 + stride + 3)[0] - (p2 - stride - 3)[0]);
                        if (nPixel > nPixelMax)
                            nPixelMax = nPixel;
        ...
        public static Bitmap Resize(Bitmap b, int nWidth, int nHeight, bool bBilinear)
        {
            ...
            if (bBilinear)
            {
                for (int x = 0; x < b.Width; ++x)
                {
                    ...
                    {
                        // Setup
                        int floor_x = (int)Math.Floor(x * nXFactor);
                        int floor_y = (int)Math.Floor(y * nYFactor);
                        ...
                        double fraction_x = x * nXFactor - floor_x;
                        double fraction_y = y * nYFactor - floor_y;

                        double one_minus_x = 1.0 - fraction_x;
                        double one_minus_y = 1.0 - fraction_y;

                        Color c1 = bTemp.GetPixel(floor_x, floor_y);
                        Color c2 = bTemp.GetPixel(ceil_x, floor_y);
                        Color c3 = bTemp.GetPixel(floor_x, ceil_y);
                        Color c4 = bTemp.GetPixel(ceil_x, ceil_y);

                        // Blue
                        byte b1 = (byte)(one_minus_x * c1.B + fraction_x * c2.B);

                        byte b2 = (byte)(one_minus_x * c3.B + fraction_x * c4.B);

                        byte blue = (byte)(one_minus_y * (double)(b1) + fraction_y * (double)(b2));
                        ...
                        byte green = (byte)(one_minus_y * (double)(b1) + fraction_y * (double)(b2));
                        ...
                        byte red = (byte)(one_minus_y * (double)(b1) + fraction_y * (double)(b2));
