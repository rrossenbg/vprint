...
namespace MerchantSite
{
    public static class MVCEx
    {
        ...
        /// <returns></returns>
        [TargetedPatchingOptOut("na")]
        public static MvcHtmlString Image(this HtmlHelper helper, string name, string path, byte[] data, object htmlAttributes = null)
        {
            var tagBuilder = new TagBuilder("img");
            tagBuilder.GenerateId(name);
            var base64 = Convert.ToBase64String(data, Base64FormattingOptions.None);
            tagBuilder.Attributes["src"] = string.Concat("data:", path.GetContentType(), ";base64,", base64);
            tagBuilder.MergeAttributes(new RouteValueDictionary(htmlAttributes));
            return new MvcHtmlString(tagBuilder.ToString());
        }
        ...
        [TargetedPatchingOptOut("na")]
        public static IEnumerable<string> GetTags(this string html, string tag = "h1")
        {
            string regexString = string.Format("<{0}[^>]*?>(?<TagText>.*?)</{0}>", tag);
            MatchCollection mc = Regex.Matches(html, regexString, RegexOptions.Multiline);
            foreach (Match m in mc)
                yield return m.Groups["TagText"].Value;
        }

        [TargetedPatchingOptOut("na")]
        public static string ReplaceTags(this string html, string tag, MatchEvaluator funct)
        {
            string regexString = string.Format("</?{0}((\\s+\\w+(\\s*=\\s*(?:\".*?\"|'.*?'|[^'\">\\s]+))?)+\\s*|\\s*)/?>", tag);
            string result = Regex.Replace(html, regexString, funct, RegexOptions.Multiline | RegexOptions.Compiled | RegexOptions.IgnoreCase);
            return result;
        }
    }
}