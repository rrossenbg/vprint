...
namespace ServiceControlManager
{
	/// <summary>
	...
	/// NOTE: Future enhancements need to be implemented to the service such that it runs the services asynchronously rather
	/// than in a synchronous manner. This will improve the performance and also the fact that a a second service may not be 
	/// skipped because of a long-running service.
	/// </summary>
	public class ServiceControlManager : System.ServiceProcess.ServiceBase
	{
		/// <summary> 
		...
		public ServiceControlManager()
		{
			...
			CreateEventSource();
			eventLog.Source = "ServiceControlManager";
		...
		private void CreateEventSource()
		{
			...
			if(!EventLog.SourceExists("ServiceControlManager"))
			{
				// An event log source should not be created and immediately used.
				// There is a latency time to enable the source, it should be created
				// prior to executing the application that uses the source.
				// Execute this sample a second time to use the new source.
				EventLog.CreateEventSource("ServiceControlManager", "ServiceControlManagerLog");
		...
		static void Main()
		{
			...
			//
			ServicesToRun = new System.ServiceProcess.ServiceBase[] { new ServiceControlManager() };
		...
		private void InitializeComponent()
		{
			...
			// 
			this.ServiceName = "ServiceControlManager";
		...
		{
			eventLog.WriteEntry("Starting ServiceControlManager...");
		}
		...
		{
			eventLog.WriteEntry("Stopping ServiceControlManager");
		}

		private void timer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
		{
			System.Diagnostics.Debug.WriteLine(e.SignalTime.ToString());
			...
			for(int i = 1; i <= int.Parse(count); i++)
			{
				try
				{
					...
					{
						System.Diagnostics.Debug.WriteLine("Time to run:", timeToRun);

						if((DateTime.Parse(timeToRun) >= System.DateTime.Now.AddMinutes(-1))
							&& (DateTime.Parse(timeToRun) <= System.DateTime.Now.AddMinutes(1)))
						{
				...
				catch (Exception ex)
				{
					...
					try
					{
						eventLog.WriteEntry("Exception in service: " + name, EventLogEntryType.Error);
					...
					{
						System.IO.StreamWriter logFile = new System.IO.StreamWriter(logPath + String.Format("{0:dd_MM_yyyy}",System.DateTime.Now) + "_ServiceControlManager.log",false);
						logFile.WriteLine(System.DateTime.Now + ": Service = " + name);
